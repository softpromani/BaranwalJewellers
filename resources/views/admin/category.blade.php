...
<div class="card">
    <div class="card-body">
      ...
      <!-- No Labels Form -->
      <form class="row g-3" action="{{ isset($editCategory) ? route('admin.category.update', $editCategory->id) : route('admin.category.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="col-md-6">
            <label for="inputPassword4" class="form-label">Category Name</label>
            <input type="text" class="form-control" name="name" placeholder="Category Name" value="{{ isset($editCategory) ? $editCategory->name : '' }}">
        </div>
        ...
        <tbody>
            @forelse ($categories as $category)
                <tr>
                    ...
                    <td>
                        <a class="btn btn-warning" href="{{ route('admin.category.edit', $category->id) }}"><i class="bi bi-pencil-square"></i></a>
